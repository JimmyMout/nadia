...
// βάζει στην βάση την αξιολόγηση
exports.putInBaseAksiologish = function (UserAksio, result){
    console.log("ektelw xreh nekrothafth");
    // console.log(newUser);
        sql.query("INSERT INTO aksiologish( star1, star2, star3, star4, star5, star6, text, protash, ektipwsh) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);",[UserAksio.star1, UserAksio.star2, UserAksio.star3, UserAksio.star4, UserAksio.star5, UserAksio.star6, UserAksio.text, UserAksio.protash, UserAksio.ektipwsh], function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(err, null);
        }
...
// δημιουργεί πίνακα που θα καραχωρηθούν τιμές για την αξιολόγηση
exports.NewAksiologish = function (Newstar1, Newstar2, Newstar3, Newstar4, Newstar5, Newstar6 ,Newtext, Newprotash, Newektipwsh = 0, Newid = '') {
    this.star1 = Newstar1;
    this.star2 = Newstar2;
    this.star3 = Newstar3;
    this.star4 = Newstar4;
    ...
    this.star6 = Newstar6;
    this.text = Newtext ;
    this.protash = Newprotash;
    this.ektipwsh = Newektipwsh;
    this.id = Newid;
}
// μεταφορά των αξιολογήσεων απο την βάση σε συνάρτηση του controller με σκοπό την προβολή τους στο site
exports.emfanishAksio = function (result){
    sql.query("SELECT * FROM aksiologish WHERE ektipwsh = 1 ", function (err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
        }
